...
export const switchGateway = async (req: Request, res: Response) => {
  try {
    ...
    if (!gateway_name || !Object.values(PaymentGateway).includes(gateway_name as PaymentGateway)) {
    ...
    if (!result) {
        ...
        return res.status(404).json({ error: "Gateway config not found in DB. Please seed it first." });
    }
